<template>
  ...
  <div>
    <!-- 特徵 -->
    <div :class="[compact ? 'text-sm text-yellow-900 dark:text-gray-100' : 'text-yellow-900 dark:text-gray-100 text-sm sm:text-base', 'mb-2']" v-if="npc.traits && npc.traits.length">
      特徵：{{ npc.traits.join('、') }}
    </div>

    <!-- 資產 -->
    <div :class="[compact ? 'text-sm text-yellow-900 dark:text-gray-100' : 'text-yellow-900 dark:text-gray-100 text-sm sm:text-base', 'mb-2']" v-if="npc.assets && npc.assets.length">
      資產：{{ npc.assets.join('、') }}
    </div>
    ...
    <div :class="[compact ? 'grid grid-cols-1 md:grid-cols-2 gap-3 text-sm' : 'grid grid-cols-1 md:grid-cols-2 gap-4']">
      <div :class="compact ? 'text-yellow-900 dark:text-gray-100' : 'text-yellow-900 dark:text-gray-100 text-sm sm:text-base'" v-if="npc.drives">
      ...
      <div :class="compact ? 'text-yellow-900 dark:text-gray-100' : 'text-yellow-900 dark:text-gray-100 text-sm sm:text-base'" v-if="npc.skills">
